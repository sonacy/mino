...
import { AppType, ExistPath, FrontendType, ProjectName, ProjectType, ServerType } from '../contants'
...
const worksapcePromt = async (baseTargetPath: string, spaceName: string) => {
  const firstAns: IProjectJson = await inquirer.prompt(
    packageNameQuestion as any
  )
  ...
  const endResult: any = await inquirer.prompt(endQuestion as any)
